...
1. Functions inside a re-rendered component also get recreated, and if they are passed to child components, those will also rerender.
   1. Wrapping those funcs in React.useCallback will prevent the rerender of child component due to function reference equity
2. Use useMemo for computationally expensive functions
3. useReducer ensures dispatch never changes, so dispatch is not included in useCallback dependnecy list
4. Unsafedispatch = function call continues even if component is unmounted
5. To set a simple mount check, combine useRef with useEffect hook:
   1.   const mountedRef = React.useRef(false);
        React.useEffect(() => {
...
6. To set a useCallback function and only run if mounted:
   1.  const dispatch = React.useCallback((...args) => {
