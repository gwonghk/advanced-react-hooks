...
1. useReducer passes 2 args to the given reducer function
   1. 1st argument is the 'state', or the current value of the count. (AKA state)
   2. 2nd argument is the 'newState', or whatever is passed to the updater function. (AKA action / setState / dispatcher function)
      1. See that anything can be passed as this arg, doesn't have to be an 'action'
2. Spread obj with expression by ...(expression logic)
3. Passing a 3rd arg, a function, to useReducer, and it will pass the 2nd argument into that function and use the return value as initial state.
4. 
